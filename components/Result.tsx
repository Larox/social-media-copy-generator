...
export const Results = (props: propsI): JSX.Element => {
  const handleCopy = () => {
    navigator.clipboard.writeText(props.resultCopy);
  };
    ...
    <>
      {props.resultCopy ? (
        <>
          ...
          >
            Suggested Copy for Social Media Platforms
          </label>
          <div className="p-2 bg-white border border-gray-200 rounded-lg shadow">
            <button
              type="button"
              onClick={handleCopy}
              className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm p-2.5 text-center flex items-strech mr-2"
            >
              Copy Result
              <svg
                aria-hidden="true"
                className="w-4 h-4 ml-2 -mr-1 py-4"
                fill="currentColor"
                viewBox="0 0 20 20"
